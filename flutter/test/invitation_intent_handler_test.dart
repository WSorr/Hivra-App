...
void main() {
  group('InvitationIntentHandler quick fetch dedupe', () {
    test('coalesces concurrent quick fetch for same capsule', () async {
      ...
      expect(
        results[0].message,
        'Fetched invitation deliveries: 2 new event(s)',
      );
      expect(
        results[1].message,
        'Fetched invitation deliveries: 2 new event(s)',
    ...
    test(
      'skips repeated quick fetch within cooldown for same capsule',
      () async {
        var calls = 0;
        final handler = InvitationIntentHandler(
          delivery: const InvitationDeliveryService(),
    ...
    test(
      'does not apply cooldown dedupe for unknown capsule identity',
      () async {
        var calls = 0;
        final handler = InvitationIntentHandler(
          delivery: const InvitationDeliveryService(),
    ...
    test(
      'does not coalesce concurrent quick fetch for unknown capsule',
      () async {
        var calls = 0;
        final completerA = Completer<InvitationWorkerResult>();
        final completerB = Completer<InvitationWorkerResult>();
        final queue =
            Queue<Completer<InvitationWorkerResult>>()
              ..add(completerA)
              ..add(completerB);
        final handler = InvitationIntentHandler(
          delivery: const InvitationDeliveryService(),
    ...
    test(
      'backs off repeated quick fetch after timeout for same capsule',
      () async {
        var calls = 0;
  ...
  group('InvitationIntentHandler pending timeout policy', () {
    test(
      'does not auto-expire outgoing invitations without a response',
      () async {
        ...
        final handler = InvitationIntentHandler(
          ...
          fetchInvitationsQuickAction:
              () async => const InvitationWorkerResult(code: 0),
        );
    ...
    test(
      'does not auto-expire when quick fetch is skipped by cooldown',
      () async {
        String idForByte(int value) =>
            base64.encode(Uint8List.fromList(List<int>.filled(32, value)));
    ...
    test(
      'does not auto-expire when full fetch returns receive failure',
      () async {
        String idForByte(int value) =>
            base64.encode(Uint8List.fromList(List<int>.filled(32, value)));
        ...
        final handler = InvitationIntentHandler(
          ...
          fetchInvitationsAction:
              () async => const InvitationWorkerResult(code: -5),
        );
    ...
    test('includes ffi diagnostics on receive failure when available', () async {
      final handler = InvitationIntentHandler(
        delivery: const InvitationDeliveryService(),
        fetchInvitationsAction:
            () async => const InvitationWorkerResult(
              code: -5,
  ...
  group('InvitationIntentHandler send semantics', () {
    test(
      'coalesces concurrent duplicate sends for same capsule slot/peer',
      () async {
        final actions = _FakeInvitationActionsService();
        final completer = Completer<InvitationWorkerResult>();
    ...
    test(
      'allows repeated send after previous in-flight send completes',
      () async {
        final actions = _FakeInvitationActionsService();
        final handler = InvitationIntentHandler(
          actions: actions,
    ...
    test(
      'treats timeout as recorded when new pending appears after send',
      () async {
            ...
            _FakeInvitationActionsService()
              ..onSend = (_, _) async {
                projectedInvitations = <Invitation>[
                  Invitation(
                    id: base64.encode(
                      Uint8List.fromList(List<int>.filled(32, 90)),
                    ),
                    fromPubkey: base64.encode(
                      Uint8List.fromList(List<int>.filled(32, 91)),
                    ),
        ...
        expect(
          result.message,
          contains('Local pending invitation is recorded'),
        );
      },
    );
    ...
    test(
      'does not treat projection-only pending as confirmed delivery',
      () async {
              ...
              ..onSend = (_, _) async {
                projectedInvitations = <Invitation>[
                  Invitation(
                    ...
                    fromPubkey: base64.encode(
                      Uint8List.fromList(List<int>.filled(32, 92)),
                    ),
                    toPubkey: recipientB64,
                    kind: StarterKind.juice,
                    starterSlot: 2,
    ...
    test(
      'does not treat pre-existing pending as newly recorded pending',
      () async {
        final recipient = Uint8List.fromList(List<int>.filled(32, 54));
        final recipientB64 = base64.encode(recipient);
        final existingPendingId = base64.encode(
          Uint8List.fromList(List<int>.filled(32, 95)),
        );
        final projectedInvitations = <Invitation>[
          Invitation(
            id: existingPendingId,
            fromPubkey: base64.encode(
              Uint8List.fromList(List<int>.filled(32, 96)),
            ),
            toPubkey: recipientB64,
            kind: StarterKind.kick,
            starterSlot: 4,
            ...
            _FakeInvitationActionsService()
              ..onSend = (_, _) async {
                projectedInvitations = <Invitation>[
                  Invitation(
                    id: base64.encode(
                      Uint8List.fromList(List<int>.filled(32, 97)),
                    ),
                    fromPubkey: base64.encode(
                      Uint8List.fromList(List<int>.filled(32, 98)),
                    ),
                    toPubkey: recipientB64,
                    kind: StarterKind.seed,
                    starterSlot: 1,
            ...
            _FakeInvitationActionsService()
              ...
              ..onFetchQuick = () async {
                projectedInvitations = <Invitation>[
                  Invitation(
                    id: base64.encode(
                      Uint8List.fromList(List<int>.filled(32, 93)),
                    ),
                    fromPubkey: base64.encode(
                      Uint8List.fromList(List<int>.filled(32, 94)),
                    ),
                    toPubkey: recipientB64,
                    kind: StarterKind.spark,
                    starterSlot: 3,
  ...
  group('InvitationIntentHandler accept semantics', () {
    test(
      'short-circuits accept when invitation is already terminal locally',
      () async {
        ...
        invitationsLoader:
            () => <Invitation>[
              Invitation(
                ...
                toPubkey: base64.encode(
                  Uint8List.fromList(List<int>.filled(32, 65)),
                ),
                kind: StarterKind.juice,
                status: InvitationStatus.pending,
                sentAt: DateTime.now().subtract(const Duration(minutes: 1)),
    ...
    test(
      'treats transport timeout with local ledger as recorded acceptance',
      () async {
    ...
    test(
      'treats worker timeout as recorded when local accepted is projected',
      () async {
  ...
  group('InvitationIntentHandler reject semantics', () {
    ...
    test(
      'treats transport timeout with local ledger as recorded rejection',
      () async {
    ...
    test(
      'short-circuits reject when invitation is already terminal locally',
      () async {
        final invitation = Invitation(
          id: base64.encode(Uint8List.fromList(List<int>.filled(32, 71))),
          fromPubkey: base64.encode(
            Uint8List.fromList(List<int>.filled(32, 72)),
          ),
          toPubkey: null,
          kind: StarterKind.spark,
...
class _FakeInvitationActionsService extends InvitationActionsService {
  ...
  onAccept;
  Future<InvitationWorkerResult> Function(Uint8List invitationId, int reason)?
  onReject;
  int sendCalls = 0;
  ...
  InvitationWorkerResult fetchQuickResult = const InvitationWorkerResult(
    code: -5,
  );
