...
class AiPluginAuditService {
  ...
  Future<AiPluginAuditReport> auditInstalledPlugins() async {
    ...
    entries.sort(
      (left, right) => left.pluginLabel.compareTo(right.pluginLabel),
    );
  ...
  Future<AiPluginAuditEntry> _auditRecord(
    ...
    if (!exists) {
      findings.add(
        AiPluginAuditFinding(
          severity: 'critical',
          pluginLabel: label,
          title: 'Plugin package file is missing',
          detail:
              'Registry entry points to ${record.storedFileName}, but the '
    ...
    if (record.runtimeAbi != WasmPluginRuntimeService.requiredRuntimeAbi) {
      findings.add(
        AiPluginAuditFinding(
          ...
          detail:
              'Expected ${WasmPluginRuntimeService.requiredRuntimeAbi}, '
    ...
    if (record.runtimeEntryExport !=
      ...
      findings.add(
        AiPluginAuditFinding(
          ...
          detail:
              'Expected ${WasmPluginRuntimeService.requiredEntryExport}, '
