...
- Current progress:
  ...
  - Remote plugin source catalogs are now accepted only when their full JSON body SHA256 matches a host-pinned digest, so package checksums are no longer trusted solely from the same mutable catalog source.
