...
- [ ] Sender bucket/checkpoint and exact charge evidence persist in the same
      encrypted repository; capacity failure returns `retry` without bypass.
...
- [ ] `tools/review/capsule_ai_runtime_gate.sh` passes; the one remaining
      1.x feature-owned dispatch path can only disappear and no new path appears.
