...
| Portable external evidence anchor | DONE | The existing artifact owner atomically exports exact signed evidence bytes plus the identity-bound public key, while the existing replay harness performs bounded canonical parsing, signature verification, and retained-anchor continuity checks off-host | Exact merge-SHA VPS-to-Mac export and rollback/fork rejection evidence is pending; no listener, receiver, account authority, lease, credential rotation, or effect path is authorized |
