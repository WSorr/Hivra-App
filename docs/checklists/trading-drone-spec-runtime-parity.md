...
| Mandate-bound prepared exchange credential | DONE | The existing host lifecycle accepts a dedicated subaccount key only through bounded stdin, verifies its API-key hash against the active Capsule-signed mandate, and retains only one host-encrypted prepared value; the canonical session apply composes that inert commit before exact session admission | Keep the credential absent from the runner unit; exact replay completes an interrupted apply without activation, while conflicting state fails closed; only separately authorized reads or effects may consume it |
...
| Bounded deterministic VPS session | DONE | A Capsule-signed v5 admission binds an exact session start, 5-minute UI cadence, at most 288 serial cycles, existing mandate effect bounds, strategy/build identity, and one runner. Each child operation id is derived from the session and cycle index; one atomic host journal advances only after immutable result retention. Compatibility v4 remains readable/executable. | Reject early cycles, expired sessions, arbitrary child ids, state mutation, replay/fork, effect-bound overflow, concurrent authority, and rotation before terminal state. Merge-SHA VPS evidence admitted one TEST session, retained two blocked cycles with zero consumed effects, rejected an early third cycle, and completed exact cleanup without co-host drift. This unit does not install an automatic effect scheduler. |
| Prepared deterministic session activation | DONE | One explicit host command revalidates the retained signed session, exact Runner identity, time, revocation absence, and encrypted credential account binding before atomically creating or replaying the canonical active session journal. | Keep the public-shadow unit disabled/inactive; reject wrong identity/account, malformed or terminal state, revocation, and mutation. Activation performs no market capture, account read, scheduler tick, provider request, or exchange effect. |
| Bounded foreground session scheduler | DONE | One explicit host process holds a single scheduler lock, derives wait/ready/stale/terminal only from verified signed cadence plus canonical session state, checks retained revocation at least every five seconds, and serially invokes the existing deterministic-order cycle owner. | Atomically mark a missed-window session stopped so authority can rotate; never burst-catch-up; stop on any other cycle failure or terminal state; install no daemon, timer, boot enablement, retry policy, parallel market capture, or second exchange-effect path. Persistent VPS service packaging remains separate. |
...
| Intent freshness + one-event/one-effect | DONE | Stable event identity remains in the zone owner; the execution use case accepts monotonic later-bar confirmation only for the same exact event binding and at most 1 bp boundary drift, then atomically claims only a LIVE effect in the existing Capsule-scoped tracking store; TEST success is validation without an order claim | Keep backward-bar, event/binding mutation, material-drift, validation-without-claim, concurrent live duplicate, restart, isolation, bounded-journal, and single-effect-owner regressions green |
...
| Post-trade position reconciliation | DONE | The same execution owner retains exact provider `positionID`, distinguishes an exact current position from one unique fully closed history record, and persists realized/net PnL in the existing Capsule-scoped journal | Keep missing/mutated identity, malformed/conflicting history, provider failure, restart retention, and no-price-guess regressions green |
...
| Canonical solo trading cycle port | DONE | `BingxFuturesTradingCycleUseCaseService` composes the existing live-strategy, sizing, WASM intent, and exchange-execution owners; zone-pending sizing uses the exact committed entry price rather than the current quote; `TradingDroneScreen` delegates solo limit preparation to it; `executed` additionally requires the nested provider result to be successful | Keep invalid input, missing event, zone-price sizing, sizing failure, missing credential, refresh, provider rejection, contradictory outcome, and single-effect delegation regressions green. VPS product acceptance remains open in `development-control.md`. |
