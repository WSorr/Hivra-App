...
| Exchange-backed risk inputs (equity/pnl/positions) | DONE | `BingxFuturesExchangeRiskInputService` is consumed by the trading-drone execution use case using `getUserBalance/getUserPositions`; non-positive equity is unavailable evidence, test diagnostics may use the bounded fallback, and live execution fails closed when any input falls back | Keep exchange payload variant, zero-equity test fallback, and live-fallback-block tests green |
