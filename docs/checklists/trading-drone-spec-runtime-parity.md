...
| Mandate-bound one-shot account read | DONE | The existing host lifecycle launches one collected transient process with the encrypted runner and exchange credentials, reuses only the existing balance/positions/open-orders signed GETs, and validates one canonical redacted `effect=false` verdict | Keep provider payloads, persistent credential access, schedules, leases, account journals, POST/DELETE, reconciliation, and effects unavailable; require merge-SHA VPS proof and exact cleanup before closure |
