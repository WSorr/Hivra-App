...
| Mandate-bound prepared exchange credential | DONE | The existing host lifecycle accepts a dedicated subaccount key only through bounded stdin, verifies its API-key hash against the active Capsule-signed mandate, and retains only one host-encrypted prepared value | Keep the credential absent from the runner unit; merge-SHA VPS provisioning, exact replay/conflict rejection, uninstall, and co-host preservation remain completion evidence; account reads and effects remain unauthorized |
