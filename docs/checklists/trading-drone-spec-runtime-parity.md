...
| Durable public shadow stream | DONE | `BingxFuturesShadowStreamStore` atomically commits the runner identity before evidence, retains at most 256 immutable authenticated evidence files behind process-local serialization and bounded OS-lock acquisition, and resumes the sequence and predecessor hash after restart | Keep identity-pending recovery, foreign-key/committed-corruption/unbound-state rejection, evidence pending-interruption, ambiguity, committed-conflict, capacity, and concurrency regressions green; rotation, daemon scheduling, deployment, leases, receivers, account reads, and remote effects remain unauthorized |
