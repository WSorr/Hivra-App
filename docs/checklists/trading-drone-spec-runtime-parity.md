...
| Exact single-use account read | DONE | The existing v2 remote admission commitment binds the Capsule signature to one runner, account, mandate, exact ordered balance/positions/open-orders scope, and `max_uses=1`; the existing host lifecycle atomically retains `pending` before the transient GET sequence and `completed` only with canonical redacted evidence | Reject v1 account-read authority, widened/reordered scope, second provider invocation, crash-pending retry, corrupt/conflicting journal state, payload retention, persistent credential access, schedules, leases, POST/DELETE, reconciliation, and effects; require merge-SHA VPS proof and exact cleanup before closure |
