...
| Bounded deterministic remote-cycle history | DONE | The existing runner lifecycle retains immutable observation and terminal result files bound to the full `operation_id`, permits a new single-use admission only after the active cycle has a valid retained result, and returns historical replay without another effect | Keep both stores strict and capped at 4096 records; reject unbound blocked outcomes, foreign files, symlinks, capacity overflow, rotation over an unresolved cycle, corrupt prior results, and replay that changes the active mandate; checkpoint compaction and continuous scheduling remain separate work |
