...
| What is the next 1.x step? | No product pass is selected. Trading Durable Emergency Pause is complete; bounded mandate, Remote Runner/VPS, background execution, and release remain blocked until a separate explicit selection. | `roadmap.md` |
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | No product pass selected | Trading Durable Emergency Pause is complete. The existing Capsule-scoped order-tracking store and exchange-execution use case remain the sole operational-state and exchange-effect owners. | Restart, Capsule isolation, unavailable/corrupt state, and mid-flight pause regressions; full gates; protected PR; same-source packaged macOS/Android smoke. | Select one bounded product pass explicitly; mandate, Remote Runner/VPS, background execution, and release remain blocked. |
