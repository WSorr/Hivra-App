...
Runner disabled. A separate explicit activation command revalidates retained
authority and encrypted account binding and creates only canonical active session state; it starts no service, scheduler, market read, or exchange effect. Automated remote transport, VPS/24/7 scheduling, new exchange-effect routes, plugin ABI, Core, Ledger, tag, Release, and 2.0 runtime work remain blocked.
...
| What is the next 1.x step? | Prove prepared-session activation on the merge-SHA VPS without starting a scheduler or exchange effect. | This board and the Trading Drone product contract |
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | Prepared-session activation implemented | After inert Apply, one explicit host command revalidates the retained signed session, exact Runner, encrypted account binding, time, and revocation state before creating or replaying only its canonical active journal. The public-shadow unit remains disabled/inactive. | Focused command/UI regressions, runner/parity self-tests, full Flutter and Rust suites, architecture/release gates, protected PR, and post-merge gates. No scheduler, market capture, provider request, plugin ABI, Core, Ledger, tag, or Release change is included. | Obtain merge-SHA VPS evidence for activation, replay, rejection, and inactive co-host integrity; automatic scheduling remains blocked. |
