...
Current development focus: `1.x Trading Remote Runner Pass N — Encrypted
Identity Restart Continuity` is selected. It may strengthen only the existing
exact-unit ephemeral smoke to prove same-credential process restart and stream
continuity. Persistent installation, credential rotation/replacement, boot
enablement, external anchoring, account reads, leases, remote effects, release,
and 2.0 work remain unauthorized.
...
| What is the next 1.x step? | Trading Remote Runner Pass N: same encrypted identity and stream must continue across one explicit process restart. | `roadmap.md`, Trading Drone goal contract |
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | Trading Remote Runner Pass N selected | The existing exact-unit smoke must prove sequence `1` then sequence `2` across stop/start with the same encrypted credential and state. | Rotation/replacement, persistent installation, boot enablement, external anchoring, account reads, leases, and remote effects remain blocked. |
