...
| **2.0 architecture** | `V2-0` and `V2-1 / passes A-D` complete; no next pass selected | Capsule recovery now has one design-only authorization, history-acceptance, replay, and prepared-activation protocol above the existing authenticated continuity artifact; runtime implementation remains unauthorized. | Schema, 20 semantic vectors, and negative mutations bind exact scope, artifact, authority, operation identity, local base, history relation, replay, and prepared evidence without adding a runtime path. | Run a short post-Pass D consolidation decision before selecting another contract; do not infer runtime recovery, storage migration, selection, addressing, or a release from design completion. |
...
1. **P1 — post-Pass D consolidation decision:** verify one owner per recovery
   fact and no duplication across blueprint, schema, fixtures, validator,
   roadmap, registry, or generated evidence; select no later contract without a
   separately named ambiguity.
