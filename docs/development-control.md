...
Pass O — Observable Runner Identity Binding` is complete; detailed evidence
remains owned by `roadmap.md`. Persistent installation, credential
rotation/replacement, boot enablement, external anchoring, account reads,
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | No active unit | Trading Remote Runner Pass O exposed the signed public-shadow `runner_key_id` and proved exact identity continuity across one explicit stop/start boundary. | Rotation/replacement, persistent installation, boot enablement, external anchoring, account reads, leases, and remote effects remain blocked. |
