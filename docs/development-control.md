...
is in bounded merge-SHA smoke remediation. The exact-order implementation is
merged, but the first packaged smoke could not produce a fresh intent because
independent public market reads exhausted the existing intent time budget. The
first remediation parallelized independent reads but packaged smoke still
failed because the BingX adapter destroyed its HTTP keep-alive session after
every request. The existing adapter may retain one bounded provider session;
order authority, effect ownership, and timeout semantics must not change.
...
| What is the next 1.x step? | Close the bounded BingX HTTP-session remediation through gates and PR, then repeat merge-SHA simulated VPS evidence. A live order requires separate approval. | This board; detailed history remains in `roadmap.md` |
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | One Exact Order Effect smoke remediation | One signed semantic operation may issue at most one BingX POST; timeout and restart reconcile without blind replay. | Reuse one bounded adapter-owned BingX HTTP session, pass required gates, then repeat the simulated lifecycle on the new merge SHA. Live order remains separately gated. |
