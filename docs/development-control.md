...
Current development focus: no next pass is selected. `1.x Trading Remote
Runner Pass T — Mandate-Bound Prepared Exchange Credential` is complete. Its
repository implementation is merged at `3fc455b`, hidden-input remediation is
merged at `dc0fdd8`, and merge-SHA host evidence proved mandate binding,
encrypted-only retention, exact replay, fail-closed mutation, no runner access,
exact uninstall, and unchanged co-hosted workloads. Account reads, leases,
...
| What is the next 1.x step? | No next pass is selected; choose one bounded product unit explicitly. | This board; detailed history remains in `roadmap.md` |
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | Pass T complete; no next pass selected | The existing host lifecycle may prepare one dedicated subaccount credential only when its API-key hash matches the exact active Capsule-signed mandate, retain only host-encrypted state, and keep it unavailable to the runner. | Select one bounded product unit explicitly; account reads, effects, background activation, and release remain blocked. |
