...
and standalone key files grant no authority. A local host apply command now composes
hidden-input credential preparation and exact session admission while leaving the
Runner disabled. Automated remote transport, VPS/24/7 scheduling, new exchange-effect routes, plugin ABI, Core, Ledger, tag, Release, and 2.0 runtime work remain blocked.
...
| What is the next 1.x step? | Prove the prepared-session apply journey on the merge-SHA VPS; activation and scheduling remain blocked. | This board and the Trading Drone product contract |
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | Prepared-session host apply implemented | The Trading Drone exports one signed bounded session and shows the exact command for its verified Runner. The host revalidates it, prepares one encrypted account-bound credential, then admits that session while remaining disabled and inactive. | Focused command/UI regressions, runner/parity self-tests, full Flutter and Rust suites, architecture/release gates, protected PR, and post-merge gates. No SSH automation, activation, scheduler, provider route, plugin ABI, Core, Ledger, tag, or Release change is included. | Obtain merge-SHA VPS evidence for this apply journey before selecting activation; scheduler remains blocked. |
