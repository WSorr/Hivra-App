...
Current development focus: `1.x Trading Remote Runner Pass P — Persistent
Disabled Install and Exact Uninstall` is selected. It may persist only the
verified public-shadow bundle, encrypted runner-only credential, exact unit,
and its private state while keeping the unit disabled and inactive by default.
Credential rotation/replacement, boot enablement, external anchoring, account
...
| What is the next 1.x step? | Trading Remote Runner Pass P: prove one fail-closed persistent disabled install and exact uninstall lifecycle. | `roadmap.md`, Trading Drone goal contract |
...
| --- | --- | --- | --- | --- |
| **1.x maintained runtime** | Trading Remote Runner Pass P selected | The existing artifact owner must atomically install one exact disabled unit and uninstall only exact owned paths, remaining retryable after partial cleanup. | Rotation/replacement, boot enablement, external anchoring, account reads, leases, and remote effects remain blocked. |
