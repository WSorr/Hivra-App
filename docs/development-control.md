...
Current development focus: `V2-0 / pass B`, design-only owner discovery and
service-locator classification. Pass A completed one repository ownership
registry, deterministic current-architecture evidence, and a fail-closed gate;
it changed no runtime path. The post-AI-4 audit found no active 1.x integrity
...
| **2.0 architecture** | `V2-0 / pass A` complete; pass B selected, design-only | Derive owner candidates and classify service-locator/oversized owner surfaces against the canonical registry. | Pass A registry/report gate is deterministic; twelve known capability families are recorded, with trading and App Shell honestly marked `NEEDS_CONTRACT`, and runtime behavior remains unchanged. | Pass B may add discovery evidence and gates only; it may not move runtime files or invent replacement contracts/facades. |
...
1. **P1 — `V2-0 / pass B` design evidence:** close deterministic owner-candidate
   coverage and classify service-locator surfaces without moving runtime code or
   adding 2.0 contracts, DTOs, storage, facades, or executable paths to 1.x.
