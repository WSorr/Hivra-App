...
liquidity_sequence_is_canonical() {
  rg -q "final longReady = longTradeOk && normalizedRequiredSide != 'sell';" "$1" &&
    ...
    rg -q "blockerCode: 'opposite_liquidity_target_wrong_side'" "$4" &&
    rg -q "blockerCode: 'opposite_liquidity_risk_reward_insufficient'" "$4" &&
    ! rg -q 'entryPrice \+ \(risk \* riskReward\)|entryPrice - \(risk \* riskReward\)' "$4" &&
...
runner_bundle_install_is_fail_closed() {
  ...
  [ -x "$1" ] &&
    ...
    rg -q '^enable_exact_self_contained_bundle_mode\(\)' "$1" &&
    rg -Fq 'exact_artifact_dir="$(readlink -f -- "$ARTIFACT_DIR")"' "$1" &&
    rg -Fq '"$exact_artifact_dir/$LIFECYCLE_NAME"' "$1" &&
...
try:
    stage = owner.split("stage_upgrade_bundle() {", 1)[1].split("\nupgrade_disabled() {", 1)[0]
    upgrade = owner.split("upgrade_disabled() {", 1)[1].split("\nuninstall_disabled() {", 1)[0]
except IndexError:
    raise SystemExit(1)
if 'chmod 0755 "$pending_upgrade_path"' not in stage:
    raise SystemExit(1)
required = [
    ...
    'host lifecycle requires a traversable bundle directory',
    'runtime_bundle_is_dynamic_user_executable "$BUNDLE_INSTALL_PATH"',
]
if any(
    value not in owner
    if value.startswith(('--', 'host lifecycle', 'runtime_bundle'))
    else value not in upgrade
...
runner_prepared_session_activation_is_fail_closed() {
  local body
  body="$(awk '
    /^activate_prepared_session\(\) \{/ { capture = 1 }
    capture { print }
    capture && /^}/ { exit }
  ' "$1")"
  rg -q -- '--activate-prepared-session <artifact-dir>' "$1" &&
    rg -q 'activate_prepared_session()' "$1" &&
    printf '%s\n' "$body" | rg -q 'require_exact_installed_bundle' &&
    printf '%s\n' "$body" | rg -q 'read_installed_runner_key_id' &&
    printf '%s\n' "$body" | rg -q 'verify_remote_mandate_artifact' &&
    printf '%s\n' "$body" | rg -q 'bounded_deterministic_session' &&
    printf '%s\n' "$body" | rg -q 'require_remote_mandate_execution_eligible' &&
    ...
    rg -q 'tradingPreparedSessionActivationCommand' "$2" &&
...
shadow_stream_is_durable() {
  ...
  checkpoint_cleanup_line="$(rg -n 'await _deleteCheckpointedEvidence\(checkpoint\);' "$1" | head -1 | cut -d: -f1)"
    ...
    rg -q 'Future<RandomAccessFile> _acquireLock' "$1" &&
    rg -q 'lockFile\.open\(mode: FileMode\.writeOnlyAppend\)' "$1" &&
    rg -q 'handle\.lock\(FileLock\.exclusive\)' "$1" &&
    rg -q 'await lockHandle\.unlock\(\);' "$1" &&
    rg -q 'await lockHandle\.close\(\);' "$1" &&
...
if runner_prepared_session_activation_is_fail_closed \
  "$RUNNER_ARTIFACT" "$TRADING_SCREEN"; then
  pass "prepared session activation is explicit, state-only, and scheduler/effect-free"
else
...
SUPERVISOR_TASKS_MUTATION="$(mktemp)"
...
BUNDLE_UPGRADE_EXECUTION_MUTATION="$(mktemp)"
...
sed 's/^TasksMax=32$/TasksMax=16/' \
  "$RUNNER_SUPERVISOR" > "$SUPERVISOR_TASKS_MUTATION"
...
sed '/chmod 0755 "$pending_upgrade_path"/d' \
  "$RUNNER_ARTIFACT" > "$BUNDLE_UPGRADE_EXECUTION_MUTATION"
...
if public_pipeline_has_authority "$PUBLIC_MUTATION" && \
  ...
  ! liquidity_sequence_is_canonical "$TVH_RULE_ENGINE" "$LIVE_DECISION" "$ZONE_DECISION" "$LIQUIDITY_TARGET_MUTATION" "$TVH_RULE_TEST" "$ZONE_DECISION_TEST" "$TRADING_CYCLE_TEST" && \
  ...
  ! runner_supervisor_is_fail_closed "$SUPERVISOR_TASKS_MUTATION" "$SHADOW_PROBE" && \
  ...
  ! runner_bundle_upgrade_is_state_preserving "$BUNDLE_UPGRADE_EXECUTION_MUTATION" "$REMOTE_PROVISIONING" && \
